 ...
 */
package org.apache.nutch.parsefilter.qna;
...
public class QnaParseFilter implements HtmlParseFilter {
  ...
  public ParseResult filter(Content content, ParseResult parseResult,
      ...
      //Elements e1 = docs.select("a[href]").remove();
      Element root=docs.body();
      Elements questions = root.getElementsContainingOwnText​("?");
      ...
      for(Element element : questions){
          ...
          String text="\0";
            for(Element child : childs){
                if(child.text().contains(current.text())) {
                  while(true) {
                    text=text+child.text();
                    Element child1=child;
                    child=child.nextElementSibling​();
                    child1.remove();
                    if(child==null||child.text().contains(next.text())||child.tagName()=="h1")
                      break;
                  }
                  break;
                }
          ...
          String base = content.getBaseUrl()+count;
